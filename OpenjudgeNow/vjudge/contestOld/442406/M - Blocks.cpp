...
public:
    ...
    Matrix const operator*(Matrix b) const
    {
        ...
        for (int k = 0; k < mSize; k++)
            ans.setVal(i, j, (ans.getVal(i, j) + num[i][k] * b.getVal(k, j) + Mod*1000) % Mod);
        return ans;
    }
};
...
int main()
{
    ...
    while (t--)
    {
        cin >> n;
        Matrix nowMatrix(4);
        nowMatrix.setVal(0,0,2);
        nowMatrix.setVal(0,1,2);
        nowMatrix.setVal(0,2,6);
        nowMatrix.setVal(0,3,6);
        Matrix muMatrix(4);
        muMatrix.setVal(2,0,1);
        muMatrix.setVal(2,1,1);
        muMatrix.setVal(0,2,-9);
        muMatrix.setVal(1,2,1);
        muMatrix.setVal(2,2,1);
        muMatrix.setVal(3,2,5);
        muMatrix.setVal(0,3,-9);
        muMatrix.setVal(1,3,1);
        muMatrix.setVal(2,3,1);
        muMatrix.setVal(3,3,5);
